...
namespace nfrr::config::config_detail {
...
inline std::expected<T, ConfigError> numeric_from_int64(std::int64_t value) {
    ...
    if constexpr (std::is_same_v<T, bool>) {
        ...
        if (value < static_cast<std::int64_t>(Limits::min()) || value > static_cast<std::int64_t>(Limits::max())) {
 ...
 */
template <Arithmetic T>
inline std::expected<T, ConfigError> numeric_from_double(double value) {
    ...
    if constexpr (std::is_floating_point_v<T>) {
        if (value < static_cast<double>(Limits::lowest()) || value > static_cast<double>(Limits::max())) {
            return std::unexpected(ConfigError::OutOfRange);
        }
        return static_cast<T>(value);
    }
        ...
        if (truncated < static_cast<double>(Limits::min()) || truncated > static_cast<double>(Limits::max())) {
            return std::unexpected(ConfigError::OutOfRange);
        }
 ...
 */
template <Arithmetic T>
inline std::expected<T, ConfigError> numeric_from_bool(bool value) {
    if constexpr (std::is_same_v<T, bool>) {
        return value;
    }
 ...
 */
template <Arithmetic T>
inline std::expected<T, ConfigError> parse_numeric(std::string_view s) {
    T result{};
...
struct IsValidPointee {
    static constexpr bool VALUE = std::is_same_v<T, bool> || std::is_same_v<T, std::int64_t> ||
                                  std::is_same_v<T, double> || std::is_same_v<T, String> || std::is_same_v<T, Array> ||
